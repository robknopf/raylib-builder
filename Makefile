...
DEBUG_TAG := _d
RAYLIB_WASM_GRAPHICS ?= GRAPHICS_API_OPENGL_ES3
...
build_desktop_debug:
	...
	$(MAKE) -C $(RAYLIB_SRC_DIR) PLATFORM=PLATFORM_DESKTOP RAYLIB_BUILD_MODE=DEBUG RAYLIB_RELEASE_PATH=$(abspath $(DESKTOP_DEBUG_BUILD_PATH)) CUSTOM_CFLAGS="-fPIC"
...
build_desktop_release:
	...
	$(MAKE) -C $(RAYLIB_SRC_DIR) clean
	$(MAKE) -C $(RAYLIB_SRC_DIR) PLATFORM=PLATFORM_DESKTOP RAYLIB_BUILD_MODE=RELEASE RAYLIB_RELEASE_PATH=$(abspath $(DESKTOP_RELEASE_BUILD_PATH)) CUSTOM_CFLAGS="-fPIC"
	@cp $(DESKTOP_RELEASE_BUILD_PATH)/libraylib.a $(LIB_OUT_DIR)/libraylib.a

build_wasm_debug:
	@mkdir -p $(WASM_DEBUG_BUILD_PATH)
	$(MAKE) -C $(RAYLIB_SRC_DIR) clean
	$(MAKE) -C $(RAYLIB_SRC_DIR) CC=emcc PLATFORM=PLATFORM_WEB GRAPHICS=$(RAYLIB_WASM_GRAPHICS) RAYLIB_BUILD_MODE=DEBUG RAYLIB_RELEASE_PATH=$(abspath $(WASM_DEBUG_BUILD_PATH))
	@cp $(WASM_DEBUG_BUILD_PATH)/libraylib.web.a $(LIB_OUT_DIR)/libraylib$(DEBUG_TAG).wasm.a

build_wasm_release:
	@mkdir -p $(WASM_RELEASE_BUILD_PATH)
	$(MAKE) -C $(RAYLIB_SRC_DIR) clean
	$(MAKE) -C $(RAYLIB_SRC_DIR) CC=emcc PLATFORM=PLATFORM_WEB GRAPHICS=$(RAYLIB_WASM_GRAPHICS) RAYLIB_BUILD_MODE=RELEASE RAYLIB_RELEASE_PATH=$(abspath $(WASM_RELEASE_BUILD_PATH))
